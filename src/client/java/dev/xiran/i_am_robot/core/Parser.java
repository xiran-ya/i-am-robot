    ...
    static LinkedList<Token<?>> tokenize(String instruction) throws SyntaxException {
        ...
        for (String keyWord : KeyWord.keyWordStrings) {
            if (keyWord.equals(word0)) {
                Token<KeyWord> keyWordToken = new Token<>(Token.Type.KEY_WORD, KeyWord.getKeyWord(keyWord));
        ...
        if (!matchedKeyWord) {
            ...
            if (word0.matches("[a-zA-Z_$]\\w*")) {
                Token<String> token = new Token<>(Token.Type.IDENTIFIER, word0);
        ...
        for (int i = 1; i < words.length; i++) {
            ...
            if (words[i].equals("+") || words[i].equals("-") || words[i].equals("*") || words[i].equals("/") || words[i].equals("%")) {
                Token<String> token = new Token<>(Token.Type.OPERATOR, words[i]);
                tokens.addLast(token);
            } else if (c == '+' || c == '-' || Character.isDigit(c)) {
                try {
                    if (words[i].contains(".")) {
                        double value = Double.parseDouble(words[i]);
                        Token<Double> token = new Token<>(Token.Type.DOUBLE, value);
                        tokens.addLast(token);
                    } else {
                        int value = Integer.parseInt(words[i]);
                        Token<Integer> token = new Token<>(Token.Type.INT, value);
                        tokens.addLast(token);
                    }
                ...
                Token<String> token = new Token<>(Token.Type.IDENTIFIER, words[i]);
                tokens.addLast(token);
            } else {
